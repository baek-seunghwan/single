...
class mixprop(nn.Module):
        ...
        def forward(self, x, adj):
            ...
            # channel 방향으로 concat
            ho = torch.cat(out, dim=1)  # [B, (gdep+1)*C, N, T]
...
class graph_constructor(nn.Module):
        ...
        def forward(self, idx):
            if self.static_feat is None:
                ...
                nodevec1 = torch.tanh(self.lin1(self.static_feat[idx]))
                nodevec2 = torch.tanh(self.lin2(self.static_feat[idx]))

            a = torch.matmul(nodevec1, nodevec2.transpose(1, 0)) \
                - torch.matmul(nodevec2, nodevec1.transpose(1, 0))
            ...
            # ✅ k를 노드 개수(idx.size(0))를 넘지 않게 안전하게 제한
